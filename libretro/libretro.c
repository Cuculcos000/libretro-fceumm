...
static void set_user_palette(void) {
	 ...
	 * internal default palette and then chooses which one to use. */
		...
		FCEU_SetPalette();
	} else if ((current_palette == PALETTE_CUSTOM) && palette_game.exist) {
		FCEU_SetPaletteUser(palette_game.data, palette_game.nEntries);
	}
	...
	/* setup raw palette */
		...
		/* run this to update internal new palette first */
		/* since raw palette will only update the RGB lookup table directly */
		FCEU_SetPalette();
		for (i = 0; i < 64; i++) {
	...
	/* setup palette presets */
		...
		FCEU_SetPalette();
		if (current_palette <= PALETTE_INTERNAL) {
			FCEU_SetPaletteUser(palettes[current_palette].data, 64);
		}
	}
}
...
static void check_variables(bool startup) {
	...
	if (environ_cb(RETRO_ENVIRONMENT_GET_VARIABLE, &var) && var.value) {
		FSettings.DisableEmphasis = !strcmp(var.value, "enabled");
	}
...
static INLINE int get_pixel_color(const uint8 *in, const uint8 *inD) {
	...
	} else if (use_raw_palette) {
		return retro_palette[pixel & 0x3F];
	}
	return color;
}
...
void retro_deinit(void) {
	...
	stereo_filter_deinit();
	if (palette_game.data) {
		FCEU_free(palette_game.data);
